...
통합 실행의 `result.html`은 로컬 텍스트 분석용 DOM snapshot으로만 생성하며
백엔드에 업로드하지 않는다. 임시 PNG는 기존 CV 분석을 유지하기 위한 비공개
입력일 뿐이며 `output/`, 백엔드, 결과 메타데이터에 남지 않는다.
...
이 모듈의 주요 판정 기준은 **AA(4.5:1)** 이다. KWCAG 5.4.3이 AA 수준을 요구하기 때문이다.
...
|---|---|---|
| 5.4.3 텍스트 콘텐츠의 명도 대비 | 텍스트와 배경 명도 대비 4.5:1 이상 (AA) | 이미지 내 텍스트 명암비 측정 및 위반 탐지 |

rule-based-analyzer는 동일한 KWCAG 5.4.3 항목을 CSS 기반으로 검사한다. 두 모듈의 결과를 합산하면 DOM 텍스트와 이미지 텍스트 양쪽을 모두 커버하게 된다.
