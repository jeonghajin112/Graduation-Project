(() => {
  ...
  const sessionId = bridgeConfig.sessionId;
  const nonce = bridgeConfig.nonce;
  const bridgeSecret = bridgeConfig.bridgeSecret;
  const documentToken = bridgeConfig.documentToken;
  const upstreamBase = bridgeConfig.upstreamBase;
  const upstreamDocument = bridgeConfig.upstreamDocument;
  const gatewayOrigin = bridgeConfig.gatewayOrigin;
  const viewerBaseOrigin = bridgeConfig.viewerBaseOrigin;
  const hasExplicitBase = bridgeConfig.hasExplicitBase;
  ...
  const transportHeader = bridgeConfig.transportHeader;
  const maxPostBodyBytes = bridgeConfig.maxRequestBodyBytes;
...
// 클러스터: 코너 자리가 겹치는 이웃 요소의 이슈를 한 칩(host)에 모은다
  const {sortIssuesBySeverity, clusterIssuesFor, entryForIssue, leaveCluster, joinCluster} = createLiveMarkerClusters({severityRanks, severityKey});
...
const resolveIssueSnapshot = (issueIds = null) => {
  ...
  const reconcileIssueTargets = preferredIssueId => {
    ...
    if (changedIssueIds.has(lastFocusedIssueId)) focusRequestVersion += 1;
  ...
  const retryFocusedIssue = issueId => {
    const version = focusRequestVersion;
    queueMicrotask(() => {
      if (version === focusRequestVersion && lastFocusedIssueId === issueId) void focusIssue(issueId);
    });
  };
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && (!popover.hidden || pendingFocusIssueId !== null)) {
      focusRequestVersion += 1;
      pendingFocusIssueId = null;
      lastFocusedIssueId = null;
